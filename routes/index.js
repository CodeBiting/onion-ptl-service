...
router.post('/hardware', async (req, res) => {
    try {
        ...
        const networkMsgFound = controlPTL.ReceivedMessages.find(m => m.type === constantsPTL.ELECTROTEC_MSGTYPE_NETWORK);
        ...
        switch (req.body.submit) {
            ...
            case 'Save Network Distribution to DB':
                if (networkMsgFound) {
                    ...
                    const configurationParsed = controlPTL.parseNetworConfiguration(networkMsgFound.msgParsed);
            ...
            case 'Load Network Distribution from DB':
                ...
                configuration = await DataPTL.getConfiguration(poolMySql.getPromise());
...
router.get('/onion', async function (req, res, next) {
    const movArchResult = await controlPTL.getMovementsPendingToSendArch(100, 0);
    const movsToSend = await controlPTL.getMovementsPendingToSend(100, 0, 0);
    const movsReceived = await controlPTL.getMovementsReceivedFromOnion(100, 0);
...
function getShelves (configuration) {
    ...
    for (let i = 0; i < configuration.length; i++) {
        const shelfAllreadyInserted = shelves.find(s => (s.id === configuration[i].shelf.id));
