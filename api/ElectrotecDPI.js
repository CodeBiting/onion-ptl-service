...
module.exports = class ElectrotecDPI {
    ...
    constructor (dpi, timeout, ptlCallback, socketEventCallback) {
        ...
        this.client.on('data', function (data) {
            ...
            const msgReceived = ElectrotecDPI.parseReceivedData(this.dpi, this.recBuffer + data);
                ...
                if (msgReceived && msgReceived.msgParsed && msgReceived.msgParsed.length > 0) {
                    ...
                    // Atenció: En aquest cas this és el socket
                        ...
                        if (msgReceived.msgParsed[i] === undefined || msgReceived.msgParsed[i] === null || msgReceived.msgParsed[i] === '') {
                            ...
                            const [msgParsed, msgKeyPressed] = ElectrotecDPI.parseReceivedMessage(msgReceived.msgParsed[i], this.dpi);
    ...
    send (data) {
        ...
        try {
            ...
            if (this.client) {
                ...
                const message = `ElectrotecDPI : Client ${this.id} : error sending, dpi connection not available`;
                logger.error(message);
                result.result = constantsPTL.RETURN_NOT_SENT;
                result.message = message;
            }
        } catch (ex) {
            const message = `ElectrotecDPI : Client ${this.id} : exception sending ${ex}`;
            logger.error(message);
   ...
   */
    static parseReceivedData (dpi, data) {
        ...
        try {
            ...
            const splittedMsgs = data.split('\x02');
            ...
            for (let i = 0; i < splittedMsgs.length; i++) {
                if (splittedMsgs[i].length > 0) {
                    ...
                    const endField = splittedMsgs[i].indexOf('\x03');
                    if (endField !== -1) {
   ...
   */
    static buildMsgDisplayPrint (nodeId, channelNumber, displayData, ledLight, ledBlinkMode, arrows, keySound, makeBeep) {
        const regNumeric = /^[0-9]+$/; // verifiquem que és numèric sense signes
        const regOnly01 = /^[01]$/; // verifiquem que és numèric amb 0,1
        const regOnly0124 = /^[0124]$/; // verifiquem que és numèric amb 0,1,2,4
        const regOnly012345678 = /^[01245678]$/; // verifiquem que és numèric amb 0,1,2,4,5,6,7,8
        const regLightFormat = /^[01],[01],[01]$/;
        ...
        switch (dataReceived[0]) {
            ...
            case '\x36': // key pressed
                ...
                if (msgParsed.error) {
                    ...
                    const keyArray = ElectrotecDPI.parseKeys(msgParsed.keys);
   ...
   */
    static parseKeyPressed (message, dpi) {
        ...
        const keysValues = result.keys.split(',');
   ...
   */
    static parseNetworkDistributionResponse (message, dpi) {
        ...
        //      nodeId,status,tipus\x05
        //      ...
        const network = message.substring(2);
        const networkMessages = network.split('\x05');
        ...
        for (let i = 0; i < networkMessages.length; i++) {
            ...
            if (networkMessages[i].length > 0) {
                ...
                const channelData = networkMessages[i].split(',');
                ...
                for (let j = 0; j < parseInt(channelData[1]); j++) {
                    ...
                    const nodeData = networkMessages[i + j + 1].split(',');
   ...
   */
    static buildMsgRelayOutput (output) {
        // const regOutput = new RegExp('^[01]\,[01]\,[01]$');
        const regOutput = /^[01],[01],[01]$/;
  ...
  */
    ...
    static convertASCIItoHex (asciiString) {
        ...
        const result = asciiArray.map(i => {
            tempASCII = i.charCodeAt(0);
            tempHex = tempASCII.toString(16);
            return '\\x' + (tempHex.length === 1 ? '0' + tempHex : tempHex);
        });
            ...
            for (let j = 0; j < networkConfiguration.channels[i].nodes.length; j++) {
                result.configuration.push({
                    ...
                    shelf: {
                        id: 0,
                        code: shelfCode,
                        type_id: (shelfType || constantsPTL.SHELF_TYPE_PICK_TO_LIGHT), // Per defecte posem pick to light
                        type_code: ''
                    },
                    id: ptlId,
