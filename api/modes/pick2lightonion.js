...
async function reloadMovements(state) {
  ...
  let movsReceived = await state.dataPTL.getMovementsReceivedFromOnion(state.connection, 1000, 0);
...
async function addMovement(state, movement, saveMovsIntoDB) {
  ...
  let movementFound = state.movements.find(m => m.externalId === movement.externalId);
  if (movementFound) {
  ...
  let messagesToPTL = state.movements.filter(m => m.ptl.id === movement.ptl.id);
  ...
  let blinking = getBlinkMode(messagesToPTL);
  let display = getDisplayFromMessage(activeMessage);

  let result = state.controlPTL.send(null, activeMessage.ptl.id, {
...
async function delMovement(state, movement, result) {
  ...
  if (movement.externalId) {
    movementIndexFound = state.movements.findIndex(m => m.externalId == movement.externalId);
  } else {
    movementIndexFound = state.movements.findIndex(m => m.id == movement.id);
  }
  ...
  let movementFound = state.movements[movementIndexFound];
  ...
  let messagesToPTL = state.movements.filter(m => m.ptl.id === movementFound.ptl.id);
  ...
  let sendResult = state.controlPTL.send(null, movementFound.ptl.id, {
    type: constantsPTL.ELECTROTEC_MSGTYPE_DISPLAY_ACK,
    display: display,
    ledLight: color,
    ledBlinkMode: blinking,
...
async function process(state, configuration, action, data) {
  ...
  let actions = {
    ...
    'getMovement': async function() {
      return state.movements.find(m => m.id == data.id);
    },
...
async function processKeyPressed(state, configuration, ptl, keyPressed) {
    ...
    let movements = state.movements.filter(m => m.ptl.id === ptl.id);
    ...
    switch(keyPressed.keys[0]) {
      case 'V': // Confirmem el missatge, l'apaguem i el confirmem a onion
        let currentActiveMov = movements.find(m => m.active === 1);
...
async function sendMovToOnion(onionConfig, movement, state) {
  let url = onionConfig.url + 'wfevent/executeEvent';
  let data = {
...
function activeNextMessage(movements, state) {
  if (movements.length > 0) {
    ...
    let currentActiveMov = movements.findIndex(m => m.active === 1);
    ...
    let blinking = getBlinkMode(movements);
    let result = state.controlPTL.send(null, movements[newActiveMov].ptl.id, {
      type: constantsPTL.ELECTROTEC_MSGTYPE_DISPLAY_ACK,
...
function changeMessageQuantity(movements, value, state) {
  if (movements.length > 0) {
    ...
    // Enviem el missatge actiu al PTL
    let blinking = getBlinkMode(movements);
    let result = state.controlPTL.send(null, movements[currentActiveMov].ptl.id, {
      type: constantsPTL.ELECTROTEC_MSGTYPE_DISPLAY_ACK,
