   ...
   */
    ...
    static extractDpiAndShelf (configuration) {
        ...
        for (let i = 0; i < configuration.length; i++) {
            const dpiFound = dpi.find(o => (o.ip === configuration[i].dpi.ip && o.port.toString() === configuration[i].dpi.port.toString()));
            ...
            const shelfFound = shelf.find(o => (o.code === configuration[i].shelf.code));
            if (shelfFound) {
   ...
   */
    async clearConfiguration (connection) {
        ...
        try {
            ...
            logger.info(`DataPTL : clearConfiguration : delete dpi result ${JSON.stringify(truncateResult)}`);
        ...
        try {
            ...
            if (movementFound && movementFound.id) {
              ...
              ` WHERE id=${movementFound.id}`;
                ...
                logger.info(`DataPTL : saveMovementReceivedFromOnion : update msg_received result ${JSON.stringify(updateResult)}`);
              ...
              ` WHERE id=${movementFound.id}`;
                ...
                logger.info(`DataPTL : saveMovementReceivedFromOnion : insert msg_received_arch result ${JSON.stringify(insertResult)}`);
            } else {
    ...
    async delMovementReceivedFromOnion (connection, movement, result) {
        ...
        try {
            ...
            if (movementFound && movementFound.id) {
              ...
              ` WHERE id=${movementFound.id}`;
                insertResult = await connection.query(sql);
                logger.info(`DataPTL : delMovementReceivedFromOnion : insert msg_received_arch result ${JSON.stringify(insertResult)}`);

                sql = 'DELETE FROM msg_received ' +
              ` WHERE id=${movementFound.id}`;
                deleteResult = await connection.query(sql);

                logger.info(`DataPTL : delMovementReceivedFromOnion : delete msg_received result ${JSON.stringify(deleteResult)}`);
            }
        ...
        try {
            ...
            if (movementFound && movementFound.id) {
              ...
              ` WHERE id=${movementFound.id}`;
                ...
                logger.info(`DataPTL : saveErrorInMovementpendingTosendToOnion : update msg_pending_to_send result ${JSON.stringify(updateResult)}`);
              ...
              `VALUES (${movement.externalId}, '${constantsPTL.MESSAGE_ACTOR_PTL}', '${constantsPTL.MESSAGE_ACTOR_ONION}', '${JSON.stringify(movement)}', '${now}', 0)`;
                insertResult = await connection.query(sql);
                logger.info(`DataPTL : saveErrorInMovementpendingTosendToOnion : insert msg_pending_to_send result ${JSON.stringify(insertResult)}`);
            }
    ...
    async saveErrorAndNotResendInMovementPendingToSendToOnion (connection, movement, result) {
        ...
        try {
            ...
            if (movementFound && movementFound.id) {
              ...
              ` WHERE id=${movementFound.id}`;
                updateResult = await connection.query(sql);
                logger.info(`DataPTL : saveErrorAndNotResendInMovementPendingToSendToOnion : update msg_pending_to_send result ${JSON.stringify(updateResult)}`);
              ...
              `VALUES (${movement.externalId}, '${constantsPTL.MESSAGE_ACTOR_PTL}', '${constantsPTL.MESSAGE_ACTOR_ONION}', '${JSON.stringify(movement)}', '${now}', '${now}', 0)`;
                insertResult = await connection.query(sql);
                logger.info(`DataPTL : saveErrorAndNotResendInMovementPendingToSendToOnion : insert msg_pending_to_send result ${JSON.stringify(insertResult)}`);
            }
    ...
    async saveOkInMovementPendingToSendToOnion (connection, movement, result) {
        ...
        try {
            ...
            if (movementFound && movementFound.id) {
              ...
              WHERE id=${movement.id}`;
                deleteResult = await connection.query(sql);
                logger.info(`DataPTL : saveOkInMovementPendingToSendToOnion : delete msg_pending_to_send_arch result ${JSON.stringify(deleteResult)}`);
            } else {
              ...
              `VALUES (${movement.id}, ${movement.externalId}, '${constantsPTL.MESSAGE_ACTOR_PTL}', '${constantsPTL.MESSAGE_ACTOR_ONION}', '${JSON.stringify(movement)}', '${now}', '${now}', 0, '${JSON.stringify(result)}')`;
                insertResult = await connection.query(sql);
                logger.info(`DataPTL : saveOkInMovementPendingToSendToOnion : insert msg_pending_to_send_arch result ${JSON.stringify(insertResult)}`);
            }
            // await connection.query('COMMIT');
        } catch (e) {
            // await connection.query('ROLLBACK');
