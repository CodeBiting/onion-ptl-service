...
const DataPTL = require('./DataPTL');
const poolMySql = require('./poolMySql');

const schedule = require('node-schedule');
// const ElectrotecDPI = require('./ElectrotecDPI');

// Paquets que implementen els diferents modes de treball
const executionMode = require(`./modes/${config.mode.name}`);
...
/**
 * Funció que s'executa cada segón per reintentar enviar els missatges sense resposta
 ...
 */
schedule.scheduleJob(config.jobs.ResendToPTL.schedule, function () {
    const msgPendingToSend = [];
    const now = (new Date()).getTime();

    try {
    // Obtenim els objectes que s'han de reenviar
        const msgToResend = ControlPTL.SentMessagesPendingAck.filter(m => (m.date.getTime() + 1000) <= now);
        ...
        for (let i = 0; i < msgToResend.length; i++) {
            ...
            const sendResult = ControlPTL.controlPTLReference.resend(msgToResend[i].ptl.id, msgToResend[i].type, msgToResend[i].message);
            ...
            if (sendResult.result !== constantsPTL.RETURN_OK) {
                ...
                const index = ControlPTL.SentMessagesPendingAck.findIndex(m => m.messageId === parseInt(msgToResend.messageId) &&
 ...
 */
schedule.scheduleJob(config.jobs.ResendToOnion.schedule, async function () {
    // const msgPendingToSend = [];
    // const now = (new Date()).getTime();

    try {
    // Obtenim els objectes que s'han de reenviar de BD
        const result = await DataPTL.getMovementsToPendingToSendToOnion(poolMySql.getPromise(), 100, 0, 1);
        if (result.status === 'OK') {
            // Enviem el missatge i el treiem de la llista de missatges per reenviar
            ...
            result.movements.forEach(async mov => {
                const movSentResult = await executionMode.process(ControlPTL.ModeState, null, 'sendMovToOnion', {
 ...
 */
schedule.scheduleJob(config.jobs.ImportOrders.schedule, function () {
    try {
    // TODO:
    } catch (e) {
...
class ControlPTL {
   ...
   */
    ...
    get ConfiguredDPIs () {
        ...
        const result = ControlPTL.DPI.map((o) => {
    ...
    static findPTL (ptlInfo) {
        ...
        try {
            ptlFound = ControlPTL.PTL.find(o => (
                o.internal_id.toString() === ptlInfo.nodeId.toString() &&
                o.channel_id.toString() === ptlInfo.channelId.toString() &&
                o.dpi.ip === ptlInfo.dpi.ip &&
                o.dpi.port.toString() === ptlInfo.dpi.port.toString()));
        } catch (e) {
   ...
   */
    ...
    */
    ...
    // Check if configured mode "picktolightonion", if not return error
        ...
        return executionMode.process(ControlPTL.ModeState, null, 'getMovement', { id });
    }
   ...
   */
    send (dpiId, ptlId, message) {
        ...
        if (ptlId) {
            ...
            ptlFound = this.configuration.find(o => (o.id.toString() === ptlId.toString()));
        }
        ...
        if (ptlFound) {
            ...
            dpiFound = ControlPTL.DPI.find(o => (o.id.toString() === dpiId.toString()));
        }
    ...
    resend (ptlId, type, message) {
        ...
        switch (type) {
            ...
            case constantsPTL.ELECTROTEC_MSGTYPE_RELAY:
                ...
                break;
            default:
                logger.error(`ControlPTL : resend : Error sending to dpi.id [${dpiFound.id}] or ptl.id [${ptlId}] message [${JSON.stringify(message)}], message.type not implemented`);
                break;
        }
   ...
   */
    receive (dataReceived, msgParsed, msgKeyPressed) {
        ...
        if (msgKeyPressed) {
            ...
            const ptlFound = ControlPTL.findPTL(msgKeyPressed);
            if (ptlFound) {
    ...
    // Save message sent to the end of the array
        ...
        if (msgParsed && msgParsed.ackType && msgParsed.ackType === constantsPTL.ELECTROTEC_ACK_OK) {
            ...
            const msgAckFound = ControlPTL.SentMessages.find(m => m.messageId === parseInt(msgParsed.messageId) &&
                                                             m.ackRecevied === false &&
            ...
            const msgAckIndexFound = ControlPTL.SentMessagesPendingAck.findIndex(m => m.messageId === parseInt(msgParsed.messageId) &&
                                                             m.ackRecevied === false &&
                                                             m.dpi.id === msgParsed.dpi.id);
    ...
    reloadConfigurationAndConnect (configuration) {
        ...
        for (let i = 0; i < configuration.length; i++) {
            ...
            if (ControlPTL.DPI.find(o => (o.ip === configuration[i].dpi.ip && o.port === configuration[i].dpi.port)) === undefined) {
                ...
                const dpi = new ElectrotecDPI(configuration[i].dpi,
                    TIMEOUT_RETRY,
                    // INTERVAL_ALIVE,
                    this.receive,
                    this.receiveEvent);
